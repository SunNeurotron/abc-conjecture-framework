...
def create_abc_universe_map():
    """Genera y guarda la figura 'Quality vs. Ramification'."""
    plt.style.use('seaborn-v0_8-whitegrid')
    fig, ax = plt.subplots(figsize=(12, 8))

    # Simular la nube de datos
    np.random.seed(42)
    ...
    # Simular los hits de alta calidad
    ...
    plt.savefig('../figures/quality_vs_rho.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
    print("  - Figura 'quality_vs_rho.png' creada.")

def create_height_plane():
    """Genera y guarda la figura 'Height Plane'."""
    plt.style.use('seaborn-v0_8-whitegrid')
    ...
    plt.savefig('../figures/height_plane.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
    print("  - Figura 'height_plane.png' creada.")

def create_impact_map():
    """Genera y guarda la figura 'Impact Map'."""
    ...
    plt.savefig('../figures/impact_map.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
...
if __name__ == '__main__':
    ...
    create_height_plane()
    create_impact_map()
    print("Todas las figuras han sido generadas en la carpeta 'figures'.")
